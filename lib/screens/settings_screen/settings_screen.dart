...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void>updateParameter(String parameter, dynamic parameterData) async {
    await FirestoreMethods().updateParameters(AuthService().currentUser!.uid, parameter , parameterData);
  }
  ...
  void updateSettingsState(List<dynamic> languages, String currentLanguage) {
    ...
    late List<Map<String,dynamic>> selected = settingsState.languageDataList.where((element) => languages.contains(element['flag'])).toList();
    late List<Map<String,dynamic>> unSelected = settingsState.languageDataList.where((element) => !languages.contains(element['flag'])).toList();
    ...
    Consumer<ColorPalette>(
      builder: (context, palette, child) {
        return Scaffold(
          ...
          body: StreamBuilder(
            stream: FirebaseFirestore.instance.doc("users/${AuthService().currentUser!.uid}").snapshots(),
                  ...
                  child: Column(
                    children: [
                                  ...
                                  children: [
                                    ...
                                    IconButton(
                                      onPressed: () {
                                        ...
                                        setState(() {
                                          userNameEditView = false;
                                        });
                                      }, 
                                      icon: const Icon(Icons.save),
                                      color: darkMode  ? Colors.grey[350] : Colors.amber[600]
                                    ),                                                   
                                  ],
                                ),
                              ),
                      ...
                      parameterCard(
                        ...
                        [const Icon(Icons.nightlight), const Icon(Icons.sunny),],
                        darkMode,
                      ),                          
                      ...
                      parameterCard(
                        palette,
                        "Sound",
                        () { updateParameter("soundOn", !soundOn);}, 
                        [const Icon(Icons.music_note),const Icon(Icons.music_off),],
                        soundOn,
                      ),
...
Widget parameterCard(ColorPalette palette, String cardBody, VoidCallback onPressed, List<Icon> iconList, var value) {
  return Padding(
    padding: const EdgeInsets.fromLTRB(16.0,8.0,16.0,8.0),
    child: SizedBox(
      width: double.infinity,
            ...
            child: Row(
              children: [
                ...
                IconButton(
                  ...
                  color: value ? const Color.fromARGB(255, 15, 214, 214) : Colors.amber[600]
                ),                            
              ],
            )
          ),
...
Widget languageCard(ColorPalette palette, String cardBody, var value, List<dynamic> languages) {
...
class _LanguageDialogState extends State<LanguageDialog> {
  ...
  void getLatestSelection(List<dynamic> languages, SettingsState settingsState) {
    late List<Map<String,dynamic>> selected = settingsState.languageDataList.where((element) => element['selected'] == true).toList();
    late List<Map<String,dynamic>> unSelected = settingsState.languageDataList.where((element) => element['selected'] == false).toList();    
    late List<Map<String,dynamic>> newList = [...selected, ...unSelected];
  ...
  Color getLanguageColor(String status, String originalSelection, ColorPalette palette) {
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  trailing: IconButton(
                    ...
                    icon: Icon(
                      language['selected'] ? Icons.remove_circle : Icons.add_circle,
                      color: widget.palette.optionButtonTextColor,
                      size: 30,
                    )
                  )
