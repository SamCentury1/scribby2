...
class TutorialHelpers {
  ...
  void listenForScoreItems(TutorialState tutorialState, AnimationState animationState) {
    // final Map<String,dynamic> step = steps[tutorialState.sequenceStep];
    final Map<String,dynamic> step = tutorialState.tutorialStateHistory2.firstWhere((element) => element['step'] == tutorialState.sequenceStep);
  ...
  void executePreviousStep(TutorialState tutorialState, BuildContext context) {
  ...
  void reactToTileTap(TutorialState tutorialState, AnimationState animationState, Map<String,dynamic> currentStep) {
      ...
      final Map<String,dynamic> nextStep = tutorialState.tutorialStateHistory2.firstWhere((element) => element['step'] == tutorialState.sequenceStep+1);
  ...
  void getFullTutorialStates2(TutorialState tutorialState, List<Map<String,dynamic>> steps) {
    ...
    for (int i=0; i < steps.length; i++) {
      ...
      if (currentStep['isTapped'] || currentStep['autoPlace']) {
        ...
        if (targetType == 'tile') {
          ...
          late Map<String,dynamic> tileState = newBoard.firstWhere((element) => element['index'] == targetIndex);

          for (int j=0; j<newBoard.length; j++ ) {

            final bool alive =  currentStep['autoPlace'] ? false : newBoard[j]['alive'];
            final bool active = currentStep['tilesInWord'].contains(newBoard[j]['index']);
          ...
          late Map<String,dynamic> reserveState = newReserves.firstWhere((element) => element['id'] == targetIndex);
        ...
        if (currentStep['isDrag']) {
          ...
          late int dragTileIndex = int.parse(currentStep['dragSource'].split("_")[1]);
          late Map<String,dynamic> reserveObject = steps[i-1]['reserves'].firstWhere((element) => element['id'] == dragTileIndex);
  ...
  Map<String,dynamic> getCurrentStep2(TutorialState tutorialState) {
    ...
    final Map<String,dynamic> currentStep = tutorialStateHistory2.firstWhere((element) => element['step'] == step);
    return currentStep;
  }    
  ...
  Map<String,dynamic> getPreviousStep2(TutorialState tutorialState) {
    ...
    final Map<String,dynamic> currentStep = tutorialStateHistory2.firstWhere(
      (element) => element['step'] == step
    );
    return currentStep;
  } 

  Color getGlowAnimationColor(Map<String,dynamic> currentStep, ColorPalette palette, dynamic widgetId, Animation animation) {
  ...
  BoxShadow getBoxShadow(Map<String,dynamic> currentStep, ColorPalette palette, dynamic widgetId, Animation animation) {
  ...
  List<Shadow> getTextShadow(Map<String,dynamic> currentStep, ColorPalette palette, dynamic widgetId, Animation animation ) {
